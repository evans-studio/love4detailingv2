-- Update all database functions to use is_booked instead of is_available

-- Drop existing function
DROP FUNCTION IF EXISTS create_anonymous_booking;

-- Recreate function with correct schema (using is_booked)
...
DECLARE
    ...
    v_booking_id UUID;
BEGIN
    -- Lock the time slot for update to prevent race conditions
    PERFORM pg_advisory_xact_lock(hashtext('time_slot_lock'::text || p_time_slot_id::text));

    -- Check if time slot is available (using is_booked)
    IF NOT EXISTS (
    ...
    -- Mark time slot as booked (using is_booked)
